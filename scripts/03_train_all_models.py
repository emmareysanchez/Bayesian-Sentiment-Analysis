...
# Transfer learning (bnn_moe → bnn_moe_hetero)
# -----------------------------

def transfer_bnn_weights(src_param_path: Path, dst_model: BayesianMoE, dst_guide):
    """
    Transfer weights from trained bnn_moe to bnn_moe_hetero.
    
    Copies parameter distributions for:
      - shared feature extractor
      - all Bayesian experts
    
    The heteroscedastic head is left with its random initialization.
    
    Args:
        src_param_path: Path to pyro_store.pt from bnn_moe training
        dst_model: BayesianMoE instance with use_heteroscedastic=True
        dst_guide: AutoNormal guide for dst_model
    """
    # Load source parameters
    src_params = torch.load(src_param_path, weights_only=False)
    
    # Transfer to destination param store
    for name, value in src_params.items():
        # Only copy expert-related params and shared extractor; skip hetero params
    ...
    print(f"Transferred {sum(1 for n in src_params.keys() if 'hetero' not in n.lower())} parameters from source model.")


def train_bnn_with_transfer(
    splits, cfg: BayesianMoEConfig, svi_cfg: SVITrainerConfig,
    device: torch.device, use_theta: bool, batch_size: int,
    transfer_from_path: Optional[Path] = None, freeze_base: bool = False,
):
    """
    Train BNN with optional transfer learning from bnn_moe.
    
    Args:
        splits: train/val/test splits
        cfg: BayesianMoEConfig
        svi_cfg: SVI training config
        device: torch device
        use_theta: whether to use topic gates
        batch_size: batch size
        transfer_from_path: Path to pyro_store.pt from base model (if None, train from scratch)
        freeze_base: if True, only train hetero head (only relevant if transferring)
    ...
    # Transfer weights if path provided
    if transfer_from_path is not None:
        print(f"Transferring weights from: {transfer_from_path}")
        transfer_bnn_weights(transfer_from_path, model, guide)
...
def train_bnn(
    ...
    """Train BNN (supports transfer learning if transfer_from_path provided)."""
    return train_bnn_with_transfer(
        splits, cfg, svi_cfg, device, use_theta, batch_size,
        transfer_from_path=transfer_from_path, freeze_base=freeze_base,
    )
...
def main():
    ...
    ap.add_argument("--transfer-from", type=str, default=None,
                    help="Model to transfer from (e.g., 'bnn_moe'). Only used with bnn_moe_hetero.")
    ap.add_argument("--transfer-seed", type=int, default=None,
                    help="Seed of source model for transfer. If None, uses first seed.")
    ap.add_argument("--freeze-base", action="store_true",
                    help="If True, freeze base model params when transferring (only train hetero head).")
    args = ap.parse_args()
    ...
    for seed in args.seeds:
        ...
        for model_name, fn_key in MODELS_TO_TRAIN:
            ...
            if fn_key == "deterministic":
                ...
                # Handle transfer learning for bnn_moe_hetero
                transfer_path = None
                if hetero and args.transfer_from:
                    src_seed = args.transfer_seed if args.transfer_seed is not None else args.seeds[0]
                    src_dir = Path(args.out_root) / args.transfer_from / f"seed_{src_seed}"
                    transfer_path = src_dir / "pyro_store.pt"
                    if not transfer_path.exists():
                        print(f"  WARNING: Transfer source not found: {transfer_path}")
                        print(f"  Training {model_name} from scratch instead.")
                        transfer_path = None
                    else:
                ...
                scaler, model, guide = train_bnn(
                    ...
                    transfer_from_path=transfer_path, freeze_base=args.freeze_base,
                )
